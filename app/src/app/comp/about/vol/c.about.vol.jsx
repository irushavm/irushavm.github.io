...
export default class VolSection extends React.Component {
  createLayout () {
    const volRecords = []
    for (let i = 0; i < this.props.record.length; i += 2) {
                ...
                <div key={`vol-set-${volRecordIndex}`} className="columns">
                  {
                    volRecordSet.map(function (vol, index) {
                      ...
                      return (
                        <a href={vol.link} target="_blank" key={`vol-${volRecordIndex}-${index}`} className="column well-on-hover vol-tile" style={{ textAlign: (index % 2 === 0) ? 'right' : 'left', padding: '20px' }} rel="noreferrer">
